...
let btnContinueB = document.querySelector(".actions .continuar.banco");
let btnVolver = document.querySelector(".actions .volver");

const transBanco = () => {
    btnContinueB.addEventListener("click", (e) => {
        ...
        for (let classN of btnContinueB.className.split(" ")) {
            if (classN === "send") {
                ...
                fetch("http://localhost:8080/dbp-project/transferencias-banco", {
                    body: data,
                    method: 'POST'
                }).then(res => {
        ...
        btnContinueB.classList.add("send");
    });
    ...
    btnVolver.addEventListener("click", (e) => {
        ...
        for (let classN of btnContinueB.className.split(" ")) {
            if (classN === "send") {
                btnContinueB.classList.remove("send");
